...
namespace Nesto.Modulos.PedidoCompra.ViewModels
{
    public class DetallePedidoCompraViewModel : BindableBase, INavigationAware
    {
        ...
        private async void OnEnviarPedido(PedidoCompraWrapper pedido)
        {
            ...
            EstaOcupado = true;
            // Nesto#340 (retirada de flags 20/08/26): MotorPdfPedidoCompra llevaba semanas al
            // 100% en QuestPDF (defecto incluido) — el RDLC local era código muerto. PDF y
            // Excel los genera SIEMPRE NestoAPI.
            byte[] pdf = await _servicioInformes.DescargarPedidoCompraPdf(pedido.Model.Empresa, pedido.Id);
            byte[] xlsx = await _servicioInformes.DescargarPedidoCompraExcel(pedido.Model.Empresa, pedido.Id);
        ...
        private async void OnImprimirPedido(PedidoCompraWrapper pedido)
        {
            ...
            // Nesto#340 (retirada de flags 20/08/26): el PDF lo genera SIEMPRE NestoAPI.
            byte[] pdf = await _servicioInformes.DescargarPedidoCompraPdf(pedido.Model.Empresa, pedido.Id);
