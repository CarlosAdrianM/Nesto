...
namespace Nesto.Informes
{
    public class MontarKitProductosModel
    {
        ...
        public static async Task<List<MontarKitProductosModel>> CargarDatos(int traspaso)
        {
            ...
            {
                string consulta = $@"select rtrim(p.Número) Producto, rtrim(p.Nombre) Nombre, p.Tamaño, rtrim(p.UnidadMedida) UnidadMedida, rtrim(p.familia) Familia, Cantidad, rtrim(Pasillo) Pasillo, rtrim(Fila) Fila, rtrim(Columna) Columna, rtrim(CodBarras) CodigoBarras
