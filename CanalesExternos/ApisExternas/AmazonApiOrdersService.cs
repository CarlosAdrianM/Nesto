...
public class AmazonApiOrdersService
{
    ...
    internal class ResultadoFeedAmazon
    {
        public EstadoFeedAmazon Estado { get; set; }
        public string Detalle { get; set; }
    }

    /// <summary>
    /// Traduce el estado del feed y su informe de procesamiento a lo que ve el usuario. Puro, para
    /// poder testearlo sin Amazon. DONE solo es "confirmado" si el informe se ha leído y no trae errores:
    /// el bug de CTT era justo un DONE con el mensaje rechazado dentro del informe.
    /// </summary>
    internal static ResultadoFeedAmazon InterpretarResultadoFeed(Feed.ProcessingStatusEnum? estado, ProcessingReportMessage informe)
    {
        var errores = (informe?.Result ?? new List<Result>())
            .Where(r => string.Equals(r?.ResultCode?.Trim(), "Error", StringComparison.OrdinalIgnoreCase))
            .Select(DescribirResultado)
            .ToList();
        var avisos = (informe?.Result ?? new List<Result>())
            .Where(r => string.Equals(r?.ResultCode?.Trim(), "Warning", StringComparison.OrdinalIgnoreCase))
            .Select(DescribirResultado)
            .ToList();

        switch (estado)
        {
            case Feed.ProcessingStatusEnum.FATAL:
                return new ResultadoFeedAmazon
                {
                    Estado = EstadoFeedAmazon.Rechazado,
                    Detalle = errores.Any() ? string.Join(" | ", errores) : "Amazon no ha podido procesar el feed (FATAL)."
                };
            case Feed.ProcessingStatusEnum.CANCELLED:
                return new ResultadoFeedAmazon { Estado = EstadoFeedAmazon.Rechazado, Detalle = "El feed se ha cancelado en Amazon (CANCELLED)." };
            case Feed.ProcessingStatusEnum.DONE:
                if (informe == null)
                {
                    return new ResultadoFeedAmazon { Estado = EstadoFeedAmazon.SinConfirmar, Detalle = "Amazon ha procesado el feed pero no se ha podido leer su informe." };
                }
                int.TryParse(informe.ProcessingSummary?.MessagesWithError, out int conError);
                if (errores.Any() || conError > 0)
                {
                    return new ResultadoFeedAmazon
                    {
                        Estado = EstadoFeedAmazon.Rechazado,
                        Detalle = errores.Any() ? string.Join(" | ", errores) : $"El informe de Amazon indica {conError} mensaje(s) con error."
                    };
                }
                return new ResultadoFeedAmazon { Estado = EstadoFeedAmazon.Confirmado, Detalle = avisos.Any() ? string.Join(" | ", avisos) : null };
            default:
                return new ResultadoFeedAmazon { Estado = EstadoFeedAmazon.SinConfirmar };
        }
    }

    private static string DescribirResultado(Result r)
    {
        string codigo = string.IsNullOrWhiteSpace(r.ResultMessageCode) ? string.Empty : $"[{r.ResultMessageCode.Trim()}] ";
        return codigo + (r.ResultDescription?.Trim() ?? "(sin descripción)");
    }
